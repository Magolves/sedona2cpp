...
public class AddDestructors extends CompilerStep {
    ...
    public void enterType(TypeDef t) {
        super.enterType(t);
        if (TranslationUtil.hasVirtualMethods(t)  || TranslationUtil.hasVirtualMethods(t.base)) {
            // Assemble code
            Block code = new Block(t.loc);
            Stmt.Return retStmt = new Stmt.Return(t.loc);
            code.stmts.add(retStmt);

            MethodDef slot = new MethodDef(t.loc, t, Slot.PUBLIC | Slot.VIRTUAL, Method.INSTANCE_DESTROY, NO_FACETS, ns.voidType, NO_PARAMETERS, code);
